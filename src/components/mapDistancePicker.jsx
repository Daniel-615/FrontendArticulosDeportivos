...
// CSS de Leaflet (IMPRESCINDIBLE)
import "leaflet/dist/leaflet.css"

// Fix de íconos por defecto (Vite)
...
// Fuerza a Leaflet a recalcular tamaño cuando se monta
function AutoResize() {
  const map = useMap()
  useEffect(() => {
...
/**
 * Props:
 * - defaultOrigin: { lat, lng }
 * - value: { origin, destination, distanceKm }
 * - onChange(next): { origin, destination, distanceKm }
 * - height (css string)
 * - showUseMyLocation (bool)
 */
export default function MapDistancePicker({
  defaultOrigin = { lat: 14.6349, lng: -90.5069 },
  value,
  onChange,
  height = "360px",
  showUseMyLocation = true,
}) {
  // Renderizar solo en cliente para evitar problemas con window/Leaflet
  ...
  // Sincroniza cuando cambie 'value' desde el padre
  useEffect(() => {
    if (value?.origin) setOrigin(value.origin)
    if (Object.prototype.hasOwnProperty.call(value ?? {}, "destination")) {
      setDestination(value?.destination ?? null)
    }
  ...
  if (!isClient) {
    // Placeholder mientras monta en cliente
    return (
      <div style={{ height }} className="rounded-xl overflow-hidden border border-slate-700 bg-slate-900/20" />
    )
  }
  ...
  return (
    <div className="space-y-3">
      <div className="flex flex-wrap gap-3 items-center">
        ...
        <span className="text-slate-100 font-semibold">Distancia aprox.: {distanceKm} km</span>
      </div>

      <div style={{ height }} className="rounded-xl overflow-hidden border border-slate-700">
        <MapContainer
          key={`${center.lat},${center.lng},${zoom}`} // fuerza re-render en cambios grandes
